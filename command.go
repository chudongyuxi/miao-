...
var (
	...
	engine = control.Register("qunmaoniang", &ctrl.Options[*zero.Ctx]{
		DisableOnDefault: false,
		Brief:            "一群一天一夫一妻制专属猫娘",
		Help:
		PrivateDataFolder: "qunmaoniang",
...
func init() {
	engine.OnFullMatch("领猫娘", zero.OnlyGroup, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			...
			case userInfo.User == uid: // 娶过别人
				ctx.SendChain(
					message.At(uid),
					message.Text("\n今天你在", userInfo.Updatetime, "领养了一只猫娘"),
			...
			if len(qqgrouplist) == 1 {
				ctx.SendChain(message.Text("~群里没有ta人没有主人了哦 明天再试试叭"))
				return
			}
			// 随机抽娶
			...
			if fiancee == uid { // 如果是自己
				...
				case 1:
					...
					ctx.SendChain(message.Text("今日你无法领养猫娘哦QWQ"))
				default:
					ctx.SendChain(message.Text("呜...没领到，你可以再尝试一次"))
					return
				}
			}
			// 去民政局办证
	...
	engine.OnFullMatch("群猫娘列表", zero.OnlyGroup, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			err := 民政局.开门时间(gid)
			if err != nil {
			...
			number := len(list)
			if number <= 0 {
				ctx.SendChain(message.Text("今天还没有人领养猫娘哦"))
				return
			}
			/***********设置图片的大小和底色***********/
			fontSize := 50.0
			...
			if err != nil {
				ctx.SendChain(message.Text("[qunmaoniang]ERROR: ", err))
			}
			/***********设置字体颜色为黑色***********/
			canvas.SetRGB(0, 0, 0)
			/***********设置字体大小,并获取字体高度用来定位***********/
			...
			sl, h := canvas.MeasureString("群猫娘列表")
			/***********绘制标题***********/
			canvas.DrawString("群猫娘列表", (1500-sl)/2, 160-h) // 放置在中间位置
