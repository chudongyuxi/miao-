...
func init() {
	...
	engine.OnRegex(`^买礼物给\s?(\[CQ:at,qq=(\d+)\]|(\d+))`, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			...
			if !ok {
				ctx.SendChain(message.Text("某猫娘主人，今天你已经送过礼物了。"))
				return
			}
			// 获取好感度
			...
			// 输出结果
			if mood == 0 {
				ctx.SendChain(message.Text("你花了", moneyToFavor, "miao币买了一袋猫粮送给了ta,ta很不喜欢,你们的好感度降低至", lastfavor))
			} else {
				ctx.SendChain(message.Text("你花了", moneyToFavor, "miao币买了一袋猫粮送给了ta,ta很喜欢,你们的好感度升至", lastfavor))
			}
		})
